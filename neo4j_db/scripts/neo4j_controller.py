...
query_delete_fake_technique = "MATCH (:Campaign)-[t:targets]-(a) WHERE NOT a.name IS NOT NULL DELETE t,a"
...
class Neo4j_Controller(object):
    ...
    def get_campaign(self, actor, date_start):
        with self._driver.session() as session:
            query_retrieve_campaign = (
                f"""MATCH (campaign:Campaign {{actor: "{actor}", date_start: date("{date_start}")}})-[attributed_to:attributed_to]->(APT:APT) OPTIONAL MATCH (campaign)-[targets:targets]->(vulnerability:Vulnerability), (campaign)-[employs:employs]->(attack_vector:Technique) RETURN campaign, APT, vulnerability, attack_vector, attributed_to, targets, employs""")

            results = session.run(query_retrieve_campaign)
            return list(results)

    def get_identities(self):
        with self._driver.session() as session:
            query_retrieve_campaign = f"""MATCH (n:Identity) RETURN n LIMIT 300"""

            results = session.run(query_retrieve_campaign)
            return list(results)

    def get_actor_context(self, name):
        with self._driver.session() as session:
            query_additional_context = f"""MATCH (APT:APT {{name: "{name}"}}) OPTIONAL MATCH (APT:APT)-[
            origin:origin]->(country:Country) RETURN APT, country"""

            results = session.run(query_additional_context)
            return list(results)

    def get_additional_context_actor(self, name):
        with self._driver.session() as session:
            query_additional_context = f"""MATCH (APT:APT {{name: "{name}"}}) OPTIONAL MATCH (APT:APT)-[
            origin:origin]->(country:Country) OPTIONAL MATCH (APT:APT)-[targets:targets]->(targeted_sector:Identity) 
            OPTIONAL MATCH (APT:APT)-[uses:uses]->( tool:Tool) OPTIONAL MATCH (APT:APT)-[uses1:uses]->(
            malware:Malware) OPTIONAL MATCH (APT:APT)-[uses2:uses]->(attack_vector:Technique) RETURN APT, country, 
            targeted_sector, tool, malware, attack_vector, origin, targets, uses, uses1, uses2"""

            results = session.run(query_additional_context)
            return list(results)

    def get_additional_context_vulnerability(self, name):
        with self._driver.session() as session:
            query_additional_context = f"""MATCH (vulnerability:Vulnerability {{name: "{name}"}})
                                            OPTIONAL MATCH (vulnerability:Vulnerability)<-[vulnerable_to:vulnerable_to]-(version:Version)
                                            OPTIONAL MATCH (version:Version)<-[has:has]-(product:Product)
                                            RETURN vulnerability, version, product, vulnerable_to, has"""
